...
-- Xdg Menu
local xdg_menu = require("gentoomenu")
-- Lain
...
----4.3----if awesome.startup_errors then
----4.3----    naughty.notify({ preset = naughty.config.presets.critical,
----4.3----                     title = "Oops, there were errors during startup!",
----4.3----                     text = awesome.startup_errors })
----4.3----end
----4.3----
----4.3------ Handle runtime errors after startup
----4.3----do
----4.3----    local in_error = false
----4.3----    awesome.connect_signal("debug::error", function (err)
----4.3----        -- Make sure we don't go into an endless error loop
----4.3----        if in_error then return end
----4.3----        in_error = true
----4.3----
----4.3----        naughty.notify({ preset = naughty.config.presets.critical,
----4.3----                         title = "Oops, an error happened!",
----4.3----                         text = tostring(err) })
----4.3----        in_error = false
----4.3----    end)
----4.3----end
-- }}}
...
-- }}}
--4.3--local function set_wallpaper(s)
--4.3--    -- Wallpaper
--4.3--    if beautiful.wallpaper then
--4.3--        local wallpaper = beautiful.wallpaper
--4.3--        -- If wallpaper is a function, call it with the screen
--4.3--        if type(wallpaper) == "function" then
--4.3--            wallpaper = wallpaper(s)
--4.3--        end
--4.3--        gears.wallpaper.maximized(wallpaper, s, true)
--4.3--    end
--4.3--end
--4.3--
--4.3---- Re-set wallpaper when a screen's geometry changes (e.g. different resolution)
--4.3--screen.connect_signal("property::geometry", set_wallpaper)
--4.3--
--4.3--awful.screen.connect_for_each_screen(function(s)
--4.3--    -- Wallpaper
--4.3--    set_wallpaper(s)
--4.3--end)
...
touchicon:buttons(gears.table.join (
          awful.button({}, 1, function()
            awful.spawn.easy_async_with_shell("xinput list-props bcm5974 | awk 'NR==2 {printf(\"%d\\n\",$4)}'",
            function(stdout)
...
-- local gentoomark = wibox.widget.textbox()
-- gentoomark:set_markup(markup.fontfg("Cantarell 9.6", "#ffffff", "entoo  "))
...
awful.keyboard.append_global_keybindings({
    ...
    awful.key({ modkey, "Ctrl"    }, "o",     function () awful.tag.incmwfact( 0.004)         end,
              {description = "slightly increase master width factor", group = "layout"}),
    awful.key({ modkey, "Ctrl"    }, "i",     function () awful.tag.incmwfact(-0.004)         end,
              {description = "slightly decrease master width factor", group = "layout"}),
...
--4.3---- Bind all key numbers to tags.
--4.3---- Be careful: we use keycodes to make it works on any keyboard layout.
--4.3---- This should map on the top row of your keyboard, usually 1 to 9.
--4.3--for i = 1, 9 do
--4.3--    globalkeys = gears.table.join(globalkeys,
--4.3--        -- View tag only.
--4.3--        awful.key({ modkey }, "#" .. i + 9,
--4.3--                  function ()
--4.3--                        local screen = awful.screen.focused()
--4.3--                        local tag = screen.tags[i]
--4.3--                        if tag then
--4.3--                           tag:view_only()
--4.3--                        end
--4.3--                  end,
--4.3--                  {description = "view tag #"..i, group = "tag"}),
--4.3--        -- Toggle tag display.
--4.3--        awful.key({ modkey, "Control" }, "#" .. i + 9,
--4.3--                  function ()
--4.3--                      local screen = awful.screen.focused()
--4.3--                      local tag = screen.tags[i]
--4.3--                      if tag then
--4.3--                         awful.tag.viewtoggle(tag)
--4.3--                      end
--4.3--                  end,
--4.3--                  {description = "toggle tag #" .. i, group = "tag"}),
--4.3--        -- Move client to tag.
--4.3--        awful.key({ modkey, "Shift" }, "#" .. i + 9,
--4.3--                  function ()
--4.3--                      if client.focus then
--4.3--                          local tag = client.focus.screen.tags[i]
--4.3--                          if tag then
--4.3--                              client.focus:move_to_tag(tag)
--4.3--                          end
--4.3--                     end
--4.3--                  end,
--4.3--                  {description = "move focused client to tag #"..i, group = "tag"}),
--4.3--        -- Toggle tag on focused client.
--4.3--        awful.key({ modkey, "Control", "Shift" }, "#" .. i + 9,
--4.3--                  function ()
--4.3--                      if client.focus then
--4.3--                          local tag = client.focus.screen.tags[i]
--4.3--                          if tag then
--4.3--                              client.focus:toggle_tag(tag)
--4.3--                          end
--4.3--                      end
--4.3--                  end,
--4.3--                  {description = "toggle focused client on tag #" .. i, group = "tag"})
--4.3--    )
--4.3--end
...
--      {description = "touchpad off", group = "extra"}),
    ...
    awful.key({}, "XF86MonBrightnessUp",
        function ()
            os.execute(string.format("xbacklight -ctrl acpi_video0 -inc 5"))
        end,
        {description = "brightness up", group = "extra"}),
    awful.key({}, "XF86MonBrightnessDown",
        function ()
            os.execute(string.format("xbacklight -ctrl acpi_video0 -dec 5"))
        end,
...
--4.3--clientbuttons = gears.table.join(
--4.3--    awful.button({ }, 1, function (c)
--4.3--        c:emit_signal("request::activate", "mouse_click", {raise = true})
--4.3--    end),
--4.3--    awful.button({ modkey }, 1, function (c)
--4.3--        c:emit_signal("request::activate", "mouse_click", {raise = true})
--4.3--        awful.mouse.client.move(c)
--4.3--    end),
--4.3--    awful.button({ modkey }, 3, function (c)
--4.3--        c:emit_signal("request::activate", "mouse_click", {raise = true})
--4.3--        awful.mouse.client.resize(c)
--4.3--    end)
--4.3--)
